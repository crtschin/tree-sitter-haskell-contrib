...
Each ERRORs today. They are **not** wired into any gate (nothing globs this dir),
so they document the bug without failing CI. Both live in the delicate
`trailing_sections` / `_item_sep` / GLR area the grammar has regressed in before
(a naive relaxation once took the harvest from 135 to 33), so they are deferred to
dedicated work with a full harvest + gen-corpus revalidation, not a quick fix.
...
- **`blank-in-trailing-rules.dump-simpl`** — a trailing rules/soup section cannot
  ...
  disambiguate banner→section vs banner→soup structurally, not let soup eat
  separators.
