...
// `paren (in) name`). The origin is the FINAL ` (...)` group at end of line, so
// the name-vs-origin split needs lookahead a token regex cannot express: the
// scanner (src/scanner.c) emits `rule_name` ending before that final group. The
// keyword tokens never lose a longest-match race because `rule_name`/`module`
...
export default grammar({
  ...
  // token stream, like the leading count of the next category; entries are not
  // newline-separated at the top level, so the two parses are ambiguous until
  // the separating newline arrives (it kills the "new category" stack, since a
  // category needs a name after its count). GLR defers the choice to then.
  ...
  rules: {
    ...
    // whitespace) heads a detail line. Longest-match picks between them, so no
    // scanner state is needed as long as GHC's blank separators stay empty.
